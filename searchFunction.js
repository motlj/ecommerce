				...
				success : function(results){
					...
					if (items.length > 0) {
						$.each(items, function(key, value){
							$('#searchResults').append('<div class="row"><div class="col-xs-12 col-sm-8 col-md-8 col-lg-8"><h1>' + value.product_name + '</h1><p>' + value['description'] + '</p><h3>Price: $' + value.price + '</h3><form method="GET" action="productDetails.php"><input type="hidden" name="id" value="' + value['id'] + '"><input type="submit" class="btn btn-success form-actions" value="More Details"></form></div><div class="col-xs-12 col-sm-4 col-md-4 col-lg-4"><img id="searchImage" src="' + value.image_link + '"></div></div><hr><br>');
						});						
					} else {
